...
import { Users, UserPlus, ChevronRight, AlertTriangle } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage profiles, risk levels, and key worker assignments.</p>
        </div>
        ...
        <CardContent className="p-0">
          {children && children.length > 0 ? (
                    ...
                    <TableRow key={child.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <TableCell>
                        ...
                        {child.lastKeyWorkDate && (
                          <span className="block text-xs text-slate-500 mt-0.5">Last seen: {new Date(child.lastKeyWorkDate).toLocaleDateString()}</span>
                        )}
                      </TableCell>
            ...
            <div className="text-center p-12">
              ...
              <p className="text-muted-foreground mt-1">There are currently no young people registered in the home.</p>
            </div>
          )}
        </CardContent>
      </Card>
